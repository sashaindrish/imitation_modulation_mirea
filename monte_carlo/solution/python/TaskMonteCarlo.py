...
class OutputVolume:  # выходной объём
    def __init__(self):
        self.worse = 0
        self.good = 0
...
class PriceForOne:  # цена заштуку
    def __init__(self):
        self.low_price = 0
        self.high_price = 0
...
class VariableCosts:  # переменные затраты
    def __init__(self):
...
class FixedCosts(OutputVolume):  # постоянные затраты
    def distribution(self):
        return al.uniform_distribution(self.worse, self.good, size=size, print_graph=False)


class DiscountRate(OutputVolume):  # норм дисконта
    def distribution(self):
        return al.uniform_distribution(self.worse, self.good, size=size, print_graph=False)


class ResidualValue:  # амортизация
    def __init__(self):
...
class TaskMonteCarlo:
    ...
    def net_present_value(self, q, f, r, p, cv, sn):  # Чистая приведенная стоимость
        ...
        NPV = NPV + (self.math_model(q, p, cv, f) + sn) / ((1 - i) ** self.years) - self.start_up_investment
    ...
    def calculate_irr(self, i):
        e = [self.CF[i] for number in range(self.years)]
    ...
    def print_statistics(self):
        dct = dict()
        # среднее
        dct.update({'Среднее': [self.avg_m(self.NPV), self.avg_m(self.PI), self.avg_m(self.irr)]})
        # Стандартная ошибка
        dct.update({'Стандартная ошибка': [self.stdDE(self.NPV), self.stdDE(self.PI), self.stdDE(self.irr)]})
        # медиана
        dct.update({'Медиана': [stat.median(self.NPV), stat.median(self.PI), stat.median(self.irr)]})
        # мода
        dct.update({'Мода': [stat.mode(self.NPV), stat.mode(self.PI), stat.mode(self.irr)]})
        # стандартное отклонение
        dct.update({'Стандартное отклонение': [self.stdev(self.NPV), self.stdev(self.PI), self.stdev(self.irr)]})
        # Дисперсия выборки
        dct.update({'Дисперсия выборки': [np.var(self.NPV), np.var(self.PI), np.var(self.irr)]})
        # Эксцесс
        dct.update({'Эксцесс': [kurtosis(self.NPV), kurtosis(self.PI), kurtosis(self.irr)]})
        ...
        # Миниум
        dct.update({'Миниум': [min(self.NPV), min(self.PI), min(self.irr)]})
        # Максиум
        dct.update({'Максиум': [max(self.NPV), max(self.PI), max(self.irr)]})
        # Сумма
        dct.update({'Сумма': [sum(self.NPV), sum(self.PI), sum(self.irr)]})
        # Счёт
        dct.update({'Счёт': [size, size, size]})
        for key in dct:
