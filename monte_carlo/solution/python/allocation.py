...
def normal_distribution(mu=0.5, sigma=0.1, nums=1000, print_graph=True):
    ...
    result = np.random.normal(mu, sigma, nums)
    if print_graph:
        print(result)
        # Create the bins and histogram
        count, bins, ignored = plt.hist(result, int(nums / 4), density=True)
...
def triangular_distribution(left=-3, mode=0, right=8, nums=100000, print_graph=True):
    print(
        " triangular distribution -  left= " + str(left) + " mode= " + str(mode) + " right= " + str(right) + " nums= " + \
        str(nums))
    result = np.random.triangular(left, mode, right, nums)

    if print_graph:
        print(result)
        count, bins, ignored = plt.hist(result, bins=200, density=True)
        plt.show()
    return result
...
def exponential_distribution(scale=1.0, nums=10000, print_graph=True):
    ...
    result = np.random.exponential(scale, size=nums)
    if print_graph:
        print(result)
        count, bins, ignored = plt.hist(result, bins=200, density=True)
        plt.show()
